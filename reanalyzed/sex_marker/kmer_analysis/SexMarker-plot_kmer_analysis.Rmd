...
#SAVE
png(file="/Users/miocene/Desktop/git_repos/dissertation/fodder/made_for_dissertation/figures/Figure_TK-Distribution_of_log2-male-to-female_ratio.png", width=1020, height=730)
ggplot(sex_abund_ratio, aes(x=m_div_f_abund)) +
  geom_density(alpha=.2, fill="blue") +
  geom_histogram(aes(y=..density..), bins=150) +
...
f <- ggplot(female_abunds_within_range, aes(x=f_abund)) +
  geom_histogram(aes(y=..density.., fill="female-only k-mers", alpha=0.5), bins=498, fill="dark red", alpha=0.5) +
  coord_cartesian(xlim=c(0, 200), ylim=c(0, 0.04)) +
  scale_x_continuous(breaks=seq(0, 200, 50)) +
  scale_y_continuous(breaks=seq(0, 0.04, 0.005)) +
...
p <- ggarrange(ggarrange(f, m, ncol = 2, labels = c("A", "B"), font.label=list(color="black",size=24)),
        ggarrange(a, labels = "C", font.label=list(color="black",size=24)), nrow = 2) 

#SAVE
png(file="/Users/miocene/Desktop/git_repos/dissertation/fodder/made_for_dissertation/figures/Figure_TK-kmer_abundances.png", width=1020, height=830, res=300)
p
dev.off()
```
...
```{r MandFabund, echo=FALSE}
female_hash_abund <- read_table(file="female_kmer_abunds_in_male_contigs.histo", col_names="f_hsh_abund")
male_hash_abund <- read_table(file="male_kmer_abunds_in_male_contigs.histo", col_names="m_hsh_abund")

# subset
max_abd <- 150
min_abd <- 0
f_hsh_subset <- female_hash_abund %>% filter(f_hsh_abund>=min_abd & f_hsh_abund<=max_abd)
m_hsh_subset <- male_hash_abund %>% filter(m_hsh_abund>=min_abd & m_hsh_abund<=max_abd)

# stats
mean(female_hash_abund$f_hsh_abund)
mean(male_hash_abund$m_hsh_abund)
print(paste("Female meand abund is ", mean(female_hash_abund$f_hsh_abund), ", while male mean abund is ", mean(male_hash_abund$m_hsh_abund), "or roughly ", ((mean(male_hash_abund$m_hsh_abund))/(mean(female_hash_abund$f_hsh_abund)))*100, "%", sep=""))

# plot
n_bin <- (max_abd-min_abd)/2
p2 <- ggplot() +
  #geom_histogram(data=f_hsh_subset, aes(x=f_hsh_abund, y=..density..), bins=n_bin, fill="dark red", alpha=0.5) +
  #geom_histogram(data=m_hsh_subset, aes(x=m_hsh_abund, y=..density..), bins=n_bin, fill="dark blue", alpha=0.5) +
  geom_density(data=m_hsh_subset, aes(x=m_hsh_abund, y=..density..), color="blue", fill="dark blue", alpha=0.5) +
  geom_density(data=f_hsh_subset, aes(x=f_hsh_abund, y=..density..), color="red", fill="dark red", alpha=0.5) +
  scale_x_continuous(breaks=seq(0, 150, 15)) +
  scale_y_continuous(breaks=seq(0, 0.03, 0.0025)) +
  theme(legend.position = c(0.01, 0.1), legend.background = element_rect(fill = "white")) +
  labs(x="k-mer Abundance") +
  theme(text=element_text(size=24))
p2

# save
png(file="/Users/miocene/Desktop/git_repos/dissertation/fodder/made_for_dissertation/figures/Figure_TK-distribution_male_vs_female_abundance_in_select_contigs.png", width=1020, height=830)
p2
dev.off()
