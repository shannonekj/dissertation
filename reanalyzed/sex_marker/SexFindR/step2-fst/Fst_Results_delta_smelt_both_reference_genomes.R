...
Fst_F <- read_tsv("biallelic_fst_femaleRef.weir.fst") %>% replace_na(list(WEIR_AND_COCKERHAM_FST=0)) %>% rename(scaf = CHROM, base=POS) %>% mutate(base=as.numeric(base)) %>% filter(WEIR_AND_COCKERHAM_FST >=0) %>% replace_na(list(WEIR_AND_COCKERHAM_FST=0))
...
ggsave("female_Fst_manhattan_with95_99_noMissing.pdf")
...
# looks like there's a lot of population structure or something... 
