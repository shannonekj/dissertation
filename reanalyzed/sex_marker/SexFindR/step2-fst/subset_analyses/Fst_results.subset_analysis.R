...
Fst_F <- read_tsv("femaleRef/femaleRef_RAD-Pst1_24F_24M.filtered_PASS.biallelic.subset.weir.fst") %>% replace_na(list(WEIR_AND_COCKERHAM_FST=0)) %>% rename(scaf = CHROM, base=POS) %>% mutate(base=as.numeric(base)) %>% filter(WEIR_AND_COCKERHAM_FST >=0) %>% replace_na(list(WEIR_AND_COCKERHAM_FST=0))
...
min(Fst_F_sort_cut1$`WEIR_AND_COCKERHAM_FST`) # lowest value there is 0.0838561  (5% cutoff)
...
min(Fst_F_sort_cut2$`WEIR_AND_COCKERHAM_FST`) # lowest value there is 0.270386 (1% cutoff)
...
mean(Fst_F$WEIR_AND_COCKERHAM_FST) # 0.02151325
...
ggsave("femaleRef_Fst_manhattan_with95_99_noMissing.pdf")
