# MAKE SURE YOU HAVE AN INDEXED GENOME IN THE OUT DIR
## Use: samtools faidx $ref_genome

library(tidyverse)
library(ggpubr)
setwd("/group/millermrgrp2/shannon/projects/dissertation/reanalyzed/sex_marker/SexFindR/step2-snpdensity/pruned_dataset/maleRef/snpden")
ref_genome <- "CHRR_integrated_M.fa"
ref_sex <- "maleRef"
...
table(true_SNPdensity$mean_MvF_dif > 0) #18153
table(true_SNPdensity$mean_MvF_dif < 0) #19442
table(true_SNPdensity$mean_MvF_dif == 0) #6441
...
# save results
setwd("/group/millermrgrp2/shannon/projects/dissertation/reanalyzed/sex_marker/SexFindR/step2-snpdensity/pruned_dataset/maleRef")
write_tsv(perm_with_true_p, "SNPdensity_perm_with_true_p.maleRef.sex.txt")
write_tsv(perm_with_true_p %>% select(scaf,base,mean_MvF_dif,Pvalue), "SNPdensity_perm_with_true_p_select.maleRef.sex.txt")

# want a look at proportion of scaffold - find the outlier
genome_index <- read_tsv("CHRR_integrated_M.fa.fai",col_names = F) %>% rename(scaf=X1,length=X2)
...
ggsave("sex_scaf_proportion_p001.maleRef.pdf")
...
ggsave("sex_scaf_proportion_p001.maleRef_small_1000000.pdf")
